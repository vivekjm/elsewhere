...
export type PendingBackup = {
  workspace: Workspace;
  photos: { itemId: string; data: string }[];
};
...
const array = (v: unknown): unknown[] => {
  if (!Array.isArray(v)) throw new Error("Invalid backup collection.");
  return v;
};
...
export function parseBackup(text: string): PendingBackup {
  ...
  if (wrapped) {
    ...
    for (const value of array(raw.photos ?? [])) {
      ...
      if (typeof p.itemId !== "string" || typeof p.data !== "string")
        throw new Error("Invalid photo in backup.");
      photos.push({ itemId: p.itemId, data: p.data });
    }
  }
  if (data.schemaVersion === 1 || data.schemaVersion === 2) {
    ...
    data = {
      version: 1,
      items: array(source.wardrobe).map((value) => {
        const i = record(value);
        if (i.image) {
          if (typeof i.id !== "string" || typeof i.image !== "string")
            throw new Error("Invalid wardrobe photo.");
          photos.push({ itemId: i.id, data: i.image });
        }
        return {
          ...i,
          image: "",
          category: category(i.category),
          shape: i.art === "cap" ? "hat" : i.art,
        };
      }),
      outfits: array(source.outfits).map((value) => {
        const o = record(value);
        return { ...o, items: o.itemIds };
      }),
      trips: array(source.trips).map((value) => {
        const t = record(value),
          days = array(t.days).map(record);
        if (new Set(days.map((d) => d.date)).size !== days.length)
          throw new Error("Duplicate day dates in backup.");
        return {
          ...t,
          travellers: t.travelers,
          weightLimit: t.luggageLimit,
          budget: t.budget ?? 0,
          days: Object.fromEntries(
            days.map((d) => {
              ...
              return [
                d.date,
                { ...d, outfitId: "", outfitIds: d.outfitIds, gear: d.itemIds },
              ];
            }),
          ),
          activities: days.flatMap((d) =>
            array(d.events).map((value) => {
              const a = record(value);
              return {
                ...a,
                date: d.date,
                category: activityType(a.type),
                gear: a.itemIds,
                link: a.url ?? "",
              };
            }),
          ...
          extras: t.packing,
          packed: Object.entries(record(t.packedItems))
            .filter(([, v]) => {
              if (typeof v !== "boolean")
                throw new Error("Invalid packing status.");
              return v;
            })
            .map(([k]) => k),
        };
  ...
  const workspace = workspaceSchema.parse(data),
    seen = new Set<string>();
  if (photos.length > workspace.items.length)
    throw new Error("Too many photos in backup.");
  for (const p of photos) {
    if (
      !workspace.items.some((i) => i.id === p.itemId) ||
      seen.has(p.itemId) ||
      !/^data:image\/(png|jpeg|webp);base64,[A-Za-z0-9+/]+={0,2}$/.test(
        p.data,
      ) ||
      p.data.length > 7_000_000
    )
      throw new Error("Invalid or duplicate photo in backup.");
    seen.add(p.itemId);
  }
  if (
    new TextEncoder().encode(JSON.stringify(workspace)).byteLength > 1_480_000
  )
    throw new Error("This backup exceeds the hosted workspace size limit.");
...
export async function exportBackup(
  ...
  if (includePhotos)
    for (const item of workspace.items) {
      ...
      if (!response.ok)
        throw new Error(
          `Could not read the photo for “${item.name}”. Retry, or turn off “Include photos” for a data-only backup.`,
        );
      ...
      if (
        ...
        throw new Error("An image is too large or has an unsupported type.");
      ...
      if (photos.reduce((n, p) => n + p.data.length, 0) > MAX_BACKUP_BYTES)
        throw new Error(
          "Photos exceed the 24 MB backup limit. Export a data-only backup instead.",
        );
    }
  ...
  if (new TextEncoder().encode(result).byteLength > MAX_BACKUP_BYTES)
    throw new Error(
      "Backup exceeds 24 MB. Turn off “Include photos” and try again.",
    );
  return result;
}
...
export async function prepareRestore(
  ...
  for (const photo of pending.photos) {
    ...
    form.set(
      ...
      `restored-${photo.itemId}.${match[1].split("/")[1]}`,
    );
    const r = await fetch("/api/images", {
      ...
      data = (await r.json().catch(() => ({}))) as {
        url?: string;
        error?: string;
      };
    if (!r.ok || !data.url || !imagePath(data.url))
      throw new Error(
        data.error ||
          "Could not restore a photo. Your current workspace has not been replaced.",
      );
