...
export function calendarICS(t: Trip): string {
  ...
  for (const a of t.activities) {
    ...
    if (a.time) {
      lines.push(
        `DTSTART:${a.date.replaceAll("-", "")}T${a.time.replace(":", "")}00`,
      );
      if (a.endTime)
        lines.push(
          `DTEND:${a.date.replaceAll("-", "")}T${a.endTime.replace(":", "")}00`,
        );
    } else
      lines.push(
        `DTSTART;VALUE=DATE:${a.date.replaceAll("-", "")}`,
        `DTEND;VALUE=DATE:${new Date(Date.parse(a.date) + 86400000).toISOString().slice(0, 10).replaceAll("-", "")}`,
      );
    lines.push(
      `SUMMARY:${escapeICS(a.title)}`,
      `LOCATION:${escapeICS(a.place)}`,
      `DESCRIPTION:${escapeICS([a.notes, t.days[a.date]?.stay ? `Stay: ${t.days[a.date].stay}` : ""].filter(Boolean).join("\n"))}`,
    );
    if (a.link && safeLink(a.link))
      lines.push(`URL:${a.link.replace(/[\r\n]/g, "")}`);
