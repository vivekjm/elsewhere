...
export function monthDays(month: string) {
  ...
  return Array.from({ length: 42 }, (_, i) =>
    new Date(Date.parse(first) + (i - weekday) * 86400000)
      .toISOString()
      .slice(0, 10),
  );
}
...
export function dayHasContent(day: Day) {
  return !!(
    day.title ||
    day.stay ||
    day.notes ||
    dayOutfits(day).length ||
    day.gear?.length
  );
}
/** A date edit is atomic: it cannot quietly erase accommodation, notes, outfits or events. */
export function resizeTrip(
  trip: Trip,
  start: string,
  end: string,
  shiftPlans = false,
  ...
  const next = structuredClone(trip),
    delta = shiftPlans ? daysBetween(trip.start, start) : 0;
  next.activities.forEach((a) => {
    a.date = addDays(a.date, delta);
  });
  next.days = Object.fromEntries(
    Object.entries(next.days).map(([date, data]) => [
      addDays(date, delta),
      data,
    ]),
  );
  if (
    next.activities.some((a) => a.date < start || a.date > end) ||
    Object.entries(next.days).some(
      ([date, day]) => (date < start || date > end) && dayHasContent(day),
    )
  ) {
    throw new Error(
      "These dates would remove existing plans. Shift the itinerary with the trip, extend the return date, or move the affected plans first.",
    );
  }
  next.days = Object.fromEntries(
    Object.entries(next.days).filter(([d]) => d >= start && d <= end),
  );
...
export function duplicateTrip(trip: Trip): Trip {
  ...
  copy.extras.forEach((e) => {
    e.id = uid();
    e.packed = false;
  });
  return copy;
}
export function dayActivities(trip: Trip, day: string): Activity[] {
...
export function dayItems(w: Workspace, trip: Trip, date: string) {
  ...
  const looks = [
    ...new Set([
      ...dayOutfits(day),
      ...events.map((a) => a.outfitId).filter(Boolean),
    ]),
  ];
  const gear = [
    ...new Set([...(day?.gear ?? []), ...events.flatMap((a) => a.gear)]),
  ];
  return {
    outfits: w.outfits.filter((o) => looks.includes(o.id)),
    gear: w.items.filter((i) => gear.includes(i.id)),
  };
}
