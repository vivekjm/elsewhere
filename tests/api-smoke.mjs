...
const edited=structuredClone(a.data.workspace);edited.items=[{id:'shirt',name:'Shirt',category:'Tops',weight:200,image:'',color:'#8b8b73',shape:'shirt'}];edited.outfits=[{id:'look',name:'Day look',items:['shirt'],occasion:'Everyday'}];edited.trips=[{id:'trip',name:'API isolation test',destination:'Test City',start:'2026-09-21',end:'2026-09-22',travellers:1,budget:0,currency:'EUR',weightLimit:8,activities:[],days:{},extras:[],packed:[],theme:'city',notes:''}];
...
const ar=await (await fetch(base+'/api/workspace',{headers:{Cookie:a.cookie}})).json();const br=await (await fetch(base+'/api/workspace',{headers:{Cookie:b.cookie}})).json();assert.equal(ar.workspace.trips[0].name,'API isolation test');assert.equal(ar.revision,1);assert.deepEqual(br.workspace.trips,[]);assert.deepEqual(br.workspace.items,[]);assert.deepEqual(br.workspace.outfits,[]);assert.equal(br.revision,0);
